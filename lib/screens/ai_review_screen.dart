...
class _AiReviewScreenState extends ConsumerState<AiReviewScreen> {
  ...
  Future<void> _saveReceiptToFirebase() async {
    ...
    try {
      ...
      final verificationMap = _data['verification'];
      final verificationObj = verificationMap != null
          ? Verification.fromMap(Map<String, dynamic>.from(verificationMap))
          : null;
  ...
  Widget _buildVerificationReport() {
    ...
    final trustScore = verification['trustScore'] as int? ?? 0;
    final status = verification['status'] as String? ?? 'Not Verified';
    final checks = Map<String, dynamic>.from(verification['checks'] ?? {});
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            status == 'Verified'
                                ? 'This receipt satisfies all critical checks and has a high trust confidence.'
                                : status == 'Review'
                                    ? 'Minor verification issues detected. Review recommended before saving.'
                                    : 'Critical discrepancies found. Verification has failed.',
                            style: TextStyle(fontSize: 10, color: Colors.white.withOpacity(0.5), height: 1.4),
                          ),
                        ],
                      ),
                    ),
